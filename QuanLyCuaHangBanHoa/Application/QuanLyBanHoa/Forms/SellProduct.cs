    ...
    public partial class SellProduct : Form
    {
        ...
        private void iconButton1_Click(object sender, EventArgs e)
        {
            if (dataGridViewHistory.Rows.Count > 0)
            {
                ...
                for (int i = 1; i < dataGridViewHistory.Columns.Count + 1; i++)
                {
                    xcelApp.Cells[1, i] = dataGridViewHistory.Columns[i - 1].HeaderText;
                }

                for (int i = 0; i < dataGridViewHistory.Rows.Count; i++)
                {
                    for (int j = 0; j < dataGridViewHistory.Columns.Count; j++)
                    {
                        xcelApp.Cells[i + 2, j + 1] = dataGridViewHistory.Rows[i].Cells[j].Value.ToString();
                    }
                }
